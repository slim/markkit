...
class MarkTest extends UnitTestCase {
    ...
    function testselect()
    {
		...
		$this->Mark->bookmarklet = "testbookmarklet";
		...
		$this->Mark3->bookmarklet = "testbookmarklet";
        ...
        $this->assertEqual($expected, $result);
		$options = array('owner' => "slim");
        $result   = count(Mark::select($options));
        $expected = 1;
        $this->assertEqual($expected, $result);
		$options = " where owner='slim' ";
        $result   = count(Mark::select($options));
        $expected = 1;
        $this->assertEqual($expected, $result);
		$options = " where pageUrl like 'http://localhost/dokuwiki/%' ";
        $result   = Mark::select($options);
	...
	function testcount()
	{
		...
		$expected = 3;
        $this->assertEqual($expected, $result);
		$result = $this->Mark->count(array('owner' => 'slim'));
		$expected = 1;
        $this->assertEqual($expected, $result);
	}
